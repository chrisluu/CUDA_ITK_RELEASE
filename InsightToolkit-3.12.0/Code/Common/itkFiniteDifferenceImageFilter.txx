...
::GenerateData()
{
  ...
  { 
    ...
    // do something using CUDA
    if( FDFunctionType == GradientAnisotropicDiffusion )
    {
      // call Gradient Anisotropic Diffusion Image Filter
      cudaGradientAnisotropicDiffusionImageFilter<TInputImage, TOutputImage>( this->GetInput(), this->GetOutput(), this->GetNumberOfIterations() );
    }
    else
    {
      std::cerr << "ERROR : No such finite difference function type exist..." << std::endl;
      assert(false);
      exit(0);
    }
