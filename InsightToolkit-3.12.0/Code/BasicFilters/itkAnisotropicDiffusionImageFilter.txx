...
::InitializeIteration()
{
  if( checkCUDA() )
  { 
    //std::cout << f->GetAverageGradientMagnitudeSquared() << std::endl;
    anisoParam.m_TimeStep = m_TimeStep;
    anisoParam.m_ConductanceParameter = m_ConductanceParameter;
    anisoParam.m_GradientMagnitudeIsFixed = m_GradientMagnitudeIsFixed;
    anisoParam.m_FixedAverageGradientMagnitude = m_FixedAverageGradientMagnitude;
//    anisoParam.m_ConductanceParameter = sqrt(m_ConductanceParameter * m_ConductanceParameter *
//                                             f->GetAverageGradientMagnitudeSquared() * 2.0f );
  }
  else
  {
