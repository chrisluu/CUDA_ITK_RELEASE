/*=========================================================================
  ...
  Module:    $RCSfile: itkDerivativeImageFilter.txx,v $
  Language:  C++
...
::GenerateData()
{
  ...
  // Filter
  typedef /*typename*/ DerivativeOperator<OperatorValueType, ImageDimension> OperatorType;	// tw--
  OperatorType oper;
  oper.SetDirection(m_Direction);
  oper.SetOrder(m_Order);
  oper.CreateDirectional();
  ...
  if( checkCUDA() )
  {
    ...
    //cudaDerivativeImageFilter<TInputImage, TOutputImage, OperatorType>(this->GetInput(), output, oper);
	cudaDerivativeImageFilter<InputImageType, OutputImageType, OperatorType>(this->GetInput(), output, oper);
    /*
    unsigned long int filterwidth = oper.GetSize(oper.GetDirection());
